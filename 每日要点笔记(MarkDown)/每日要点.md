...
- 参数列表:
  - 处理请求时:需要什么写什么
  - 处理异常时:必须有一个异常类型的参数,表示Spring MVC框架调用处理请求的方法时捕获的异常,并且,可以俺需要添加`httpServletRequest`、`httpServletResponse`等少量特定类型的参数,不可以随意添加其他参数

### 5.关于异常的补充:

#### 异常的顶类是Throwable,它有两个子类:Error和Exception

- ##### Rrror:用来指示运行时环境发生了错误,例如:JVM内存溢出,一般,程序不会从错误中恢复,编译时也检查不到,其脱离了程序员的控制

  - 两个子类:OutOfMemoryError和IOError
