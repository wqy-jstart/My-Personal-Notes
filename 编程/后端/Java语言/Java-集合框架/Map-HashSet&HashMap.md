...
<u>*HashMap*实现了*Map*接口，即允许放入`key`为`null`的元素，也允许插入`value`为`null`的元素</u>
...
1. 一种开放地址方式(Open addressing)
2. 另一种是冲突链表方式(Separate chaining with linked lists)。

**Java7 HashMap采用的是冲突链表方式**。

![img](https://pdai.tech/images/collection/HashMap_base.png)
...
1. 初始容量(inital capacity)：初始容量指定了初始`table`的大小
2. 负载系数(load factor)：负载系数用来指定自动扩容的临界值

当`entry`的数量超过`capacity*load_factor`时，容器将自动扩容并重新哈希。对于插入元素较多的场景，将初始容量设大可以减少重新哈希的次数。

将对象放入到*HashMap*或*HashSet*中时，有两个方法需要特别关心: 

1. `hashCode()`：`hashCode()`方法决定了对象会被放到哪个`bucket`里
2. `equals()`：当多个对象的哈希值冲突时，`equals()`方法决定了这些对象是否是“同一个对象”。

所以，如果要将自定义的对象放入到`HashMap`或`HashSet`中，需要重写`hashCode()`和`equals()`方法。

### 2.get()

`get(Object key)`方法根据指定的`key`值返回对应的`value`，该方法调用了`getEntry(Object key)`得到相应的`entry`，然后返回`entry.getValue()`。

因此`getEntry()`是算法的核心。 算法思想是首先通过`hash()`函数得到对应`bucket`的下标，然后依次遍历冲突链表，通过`key.equals(k)`方法来判断是否是要找的那个`entry`。

![img](https://pdai.tech/images/collection/HashMap_getEntry.png)
...
![img](https://pdai.tech/images/collection/HashMap_addEntry.png)
...
![img](https://pdai.tech/images/collection/HashMap_removeEntryForKey.png)
...
Java8 的源码可读性要差一些，不过精简一些。

<u>Java7 中使用 Entry 来代表每个 HashMap 中的数据节点，Java8 中使用 Node</u>，基本没有区别，都是 key，value，hash 和 next 这四个属性，不过，Node 只能用于链表的情况，红黑树的情况需要使用 TreeNode。
